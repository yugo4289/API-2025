...
![alt text](<スクリーンショット 2025-11-25 115557.png>)
2.  **READ操作（GET）**
    *   全データを取得するGETリクエストをPostmanで送信。
    *   クエリパラメータ `limit` と `offset` を使ってページネーションの動作確認を実施
![alt text](<スクリーンショット 2025-11-25 115527.png>)
3.  **条件検索（GET with Query）**
    *   `name` が「田中」のレコードだけ取得するGETリクエストを実行
![alt text](<スクリーンショット 2025-11-25 115438.png>)
4.  **CREATE操作（POST）**
    *   新しいレコード（例：`id=101, name=田中, email=test@example.com, score=85`）を追加
![alt text](<スクリーンショット 2025-11-25 103808.png>)
![alt text](<スクリーンショット 2025-11-25 120245.png>)
5.  **複数レコード追加（POST）**
    *   `data` 配列で複数レコードを一度に追加する。
![alt text](<スクリーンショット 2025-11-25 103551.png>)
![alt text](<スクリーンショット 2025-11-25 154655.png>)
6.  **DELETE操作**
